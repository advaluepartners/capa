...
export const useThemeSandbox = () => {
  const [themeConfig, setThemeConfig] = useState(defaultDark)
  ...
  useEffect(() => {
    ...
    setIsSandbox(hash.includes('#theme-sandbox') || localPreset !== null)

    if (localPreset) {
      try {
        setThemeConfig(JSON.parse(localPreset))
      } catch (e) {
        console.error('Failed to parse theme config from local storage:', e)
      }
    }

    const styles = document.querySelector(':root') as HTMLElement

    const updateCSSVariables = () => {
      Object.entries(themeConfig).forEach(([key, value]) => styles.style.setProperty(key, value))
    ...
    const init = async () => {
      if (!hash.includes('#theme-sandbox') && !localPreset) return
      ...
      Object.entries(defaultDark).forEach(([key]) => {
        ...
        folder.add(themeConfig, key).name(key).onChange((newValue) => {
  ...
  return { themeConfig, setThemeConfig, isSandbox }
}
